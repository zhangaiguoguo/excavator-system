...
export class JobsService {
  ...
  async findAll(filters?: {
    ...
    if (filters?.equipmentType)
      qb.andWhere('JSON_CONTAINS(j.equipment_types, CAST(:eq AS JSON)) = 1', {
        eq: JSON.stringify(filters.equipmentType),
      });
    if (filters?.province)
      qb.andWhere('j.province = :province', { province: filters.province });
    if (filters?.city) qb.andWhere('j.city = :city', { city: filters.city });
    if (filters?.district)
      qb.andWhere('j.district = :district', { district: filters.district });
    if (filters?.priceMin)
      qb.andWhere('j.price >= :priceMin', { priceMin: filters.priceMin });
    if (filters?.priceMax)
      qb.andWhere('j.price <= :priceMax', { priceMax: filters.priceMax });
    if (filters?.userId)
      qb.andWhere('j.user_id = :userId', { userId: filters.userId });
  ...
  async update(
    ...
    if (updateByUserId !== undefined) payload.updateBy = String(updateByUserId);
